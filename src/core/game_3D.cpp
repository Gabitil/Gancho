...
void loadEnvironmentModels(int level)
{
    ...
    // Armazena um único VBO para instâncias de spawn.
    {
        ...
        {
            if (spawnVBO.vboID != 0)
                glDeleteBuffers(1, &spawnVBO.vboID);
            spawnVBO = tmp;
            printf(
...
GameAction gameUpdate_3D()
{
    ...
    if (requestHookRelease_3D)
    {
        ...
        if (shotsRemaining_3D > 0)
        {
            ...
            if (activeLevel_3D == 2)
                heightOffsetMultiplier = 1.55f;
            else if (activeLevel_3D == 3)
                heightOffsetMultiplier = 1.8f;
...
void gameDisplay_3D()
{
    ...
    auto getTextureForObject = [&](const std::string &objectType) -> GLuint
    {
        ...
        {
            if (objectType == "spawn")
                selectedTexture = decorationTextureID;
            else if (objectType == "platform")
                selectedTexture = decorationTextureID;
            else if (objectType == "block-large")
                selectedTexture = decorationTextureID;
            else if (objectType == "spike")
                selectedTexture = decorationTextureID;
            else if (objectType == "decoration-tall2")
                selectedTexture = decorationTextureID;
            else if (objectType == "decoration-tall" || objectType == "decoration")
                selectedTexture = colormap3TextureID;
            else
                selectedTexture = decorationTextureID; // padrão
        ...
        else if (activeLevel_3D == 2)
        {
            ...
            if (objectType == "block-large")
                return decorationTextureID;
            if (objectType == "decoration-tall" || objectType == "decoration-tall2" || objectType == "decoration" ||
                objectType == "rocks" || objectType == "spike")
                return colormap3TextureID;
        ...
        {
            if (objectType == "spawn" || objectType == "rocks" || objectType == "block-large")
                return colormap2TextureID;
            if (objectType == "spike" || objectType == "door" ||
                objectType == "decoration-tall" || objectType == "decoration-tall2" || objectType == "decoration")
                return colormap3TextureID;
            return decorationTextureID; // padrão
        }
    ...
    {
        if (info.vboID == 0 || info.triCount == 0)
            return;
        if (textureID != 0)
        {
...
void loadGameTextures_3D()
{
    ...
    if (decorationTextureID != 0)
        glDeleteTextures(1, &decorationTextureID);
    if (colormap2TextureID != 0)
        glDeleteTextures(1, &colormap2TextureID);
    if (colormap3TextureID != 0)
        glDeleteTextures(1, &colormap3TextureID);

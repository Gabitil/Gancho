/**
 * Para o jogo 3D, na parte da mecânica e da física, devido à proposta do jogo ser de ensinar cinemática vetorial,
 * tivemos muitas dificuldades relacionadas a cálculos no geral e mostrar de uma forma que fique fácil de visualizar
 * e aprender o que o jogo se propõe a mostrar. Para isso, adotamos um novo padrão de projeto em que separamos
 * todas as constantes globais e fazemos os cálculos com base nos seus valores para garantir a linearidade e a fluidez.
 * Além disso, pensamos em manter a mesma jogabilidade do modo 2D, no entanto, é impossível definir corretamente a posição
 * do mouse em um mundo tridimensional. Tivemos de pensar em alternativas que mantessem o entretenimento sem perder a proposta educativa.
 * Além de algumas mudanças na mecânica e na forma de jogar, também enfrentamos dificuldades em desenhar o mundo visível de fato pois,
 * além da complexidade do próprio mundo 3D, também precisamos usar outras formas de desenho extremamente diferentes do mundo 2D, tais
 * como profundidade, perspectiva, iluminação, colisões, cálculos etc.
 * Assim como explicado na "main.cpp", todos os headers foram refeitos para garantir a linearidade das funções e libs externas que são usadas
 * na lógica. As funções que usamos (para montagem dos VBOs, desenhos de texto, checagem de colisões e etc) são funções amplamente usadas em
 * jogos, portanto, são formas otimizadas de conseguir os resultados esperados.
 */
 ...
 * seja fixado e o x (direta e esquerda) também sejam, o y (cima e baixo) ainda pode
 * se movimentar livremente e, em um mundo tridimensional, isso forma um plano. Não é
 * possível definir a posição exata da mira. O problema disso é que o gancho era solto
 * em um lugar diferente de onde a mira apontava. Para corrigir esse problema, definimos
 * uma distância da mira ao personagem para que o y fosse uma reta horizontal. Essa distância
 * define o "ponto ótimo" de disparo. Não conseguimos deixar esse valor dinâmico a depender de onde
 * atira, é uma lógica extremamente complexa.
 */
...
/**
 * Posição absoluta da Câmera. Como estamos no modo de terceira pessoa, escolheremos
 * uma visualização próxima ao player mas com algumas modificações para permitir
 * a observação dos vetores na tela
 */
...
/**
 * Ângulos de Euler (Yaw/Pitch) para controle da mira (Mouse Look). Esse controle
 * teve de ser adicionado pois, em alguns casos, o personagem poderia olhar muito
 * para cima e acabar olhando abaixo do chão. É um problema comum em jogos FPS e buscamos
 * uma boa solução para esse caso
 */
...
bool isMouseFree_3D = false;      // Variável para controlar mouse em modo FPS ou ponteiro livre
...
/**
 * Cria os VBOs para toda a geometria estática do nível 3D. Função análoga à função feita no 2D
 * para criação das display lists. Nesse caso, a forma de criar é um pouco diferente pois é necessário
 * limpar a memória, criar os VBOs e armazenar o seu valor na memória para manipulações posteriores
 */
void createLevelVBOs()
{
    // Limpeza de VBOs antigos para liberar a memória, visto que VBOs são armazenadas na CPU
...
/**
 * Cria a grade do chão para ajudar na visualização da movimentação. Usado apenas para debug
 * TODO - remover essa visualização para aplicação de texturas, serve apenas para ver a física
 */
...
/**
 * Tela de desenho da caixa de informações das constantes e vetores. Versão análoga à usada em 2D
 */
...
/**
 * Tela de desenho da caixa de informações da quantidade de disparos restante. Versão análoga à usada em 2D
 */
void drawShotCounterHUD_3D()
{
    ...
    sprintf(shotText, "Disparos: %d / %d", shotsRemaining_3D, levelParameters_3D.maxShots);
...
void drawChargeBar(int w, int h)
{
    ...
    glColor3f(1.0f, 1.0f - chargePercentage_3D, 0.0f); // Lógica de ir de amarelo até vermelho para representar melhor a força usada
...
void drawHUD()
{
    ...
    int w = glutGet(GLUT_WINDOW_WIDTH);
    int h = glutGet(GLUT_WINDOW_HEIGHT);
    gluOrtho2D(0, w, h, 0); // Y invertido (0 no topo) para bater com as coordenadas do mouse
    ...
    else
    {
        ...
        /**
         * Essa é a lógica que define onde será o tiro do gancho. Tivemos muita dificuldade em
         * encontrar uma maneira correta para esse cálculo e que funcionasse para a maioria dos casos
         */
...
/**
 * Desenha os vetores de debug. Equivalente visual ao jogo 2D, mas adaptado para o mundo tridimensional
 */
void drawDebugVectors()
{
    ...
    if (vMag > 0.1f)
    {
        float scaleVel = 0.5f; // Ou podemos usar VELOCITY_VISUAL_SCALE_3D, mas optamos pela escala local de velocidade
...
/**
 * Chamada quando o nível 3D é iniciado. Nessa função as variáveis de controle são resetadas
 * para que não hajam inconsistências na passagem de um nível para o outro.
 * Estrutura análoga ao 2D adaptada para o uso de VBOs.
 */
void gameStartLevel_3D(int level)
{
    ...
    // Força o modo FPS no início da fase e deixa o cursor sempre no centro da tela
    ...
    // Define a quantidade de tiros restantes (escolhida em cada fase) e define em qual plataforma o jogador irá começar a fase
 ...
 * Essa função, assim como no modo 2D, é o coração de todos os cálculos e mecânica do jogo
 */
GameAction gameUpdate_3D()
{
    ...
    if (requestHookRelease_3D)
    {
        ...
        else
        {
            if (shotsRemaining_3D > 0)
            {
                ...
                /**
                 * Assim como explicado no cabeçalho, esse foi o cálculo que escolhemos usar para definir onde o gancho
                 * iria atirar. Não conseguimos fazer isso ser dinâmico a depender da distância que estava olhando, portanto,
                 * fixamos uma distância de "tiro ótimo"
                 */
                ...
                hookProjectileY_3D = pCy + (player_3D.h * 0.6f) + (dirViewY * spawnDist);
    ...
    if (isHookFiring_3D)
    {
        ...
        else
        {
            ...
            {
                /**
                 * Um problema persistente foi o de atirar para cima e o gancho se prender no chão. Para contornar isso,
                 * ignoramos a plataforma que o jogador está encostando
                 */
    ...
    if (isHooked_3D)
    {
        ...
        if (currDist > 0.1f)
        {
            ...
            // Adiciona aceleração na direção do gancho baseada na carga e na constante de aceleração definida na fase
            ...
            // Uma das principais lógicas do gancho: O jogador deve se aproximar do ponto de disparo
            ...
            if (currDist > ropeLength_3D)
            {
                ...
                // Caso o jogador se afaste do ponto que está preso ao gancho, sua posição é atualizada
                player_3D.x += normX * stretch;
                player_3D.y += normY * stretch;
                player_3D.z += normZ * stretch;

                // Corrige a velocidade do plater, anulando a velocidade de afastamento do ponto que está preso no gancho
            ...
            // Se a força Y for positiva e forte, tira o status de "Grounded" para evitar que o atrito do chão freie o puxão inicial
    ...
    // Cálculo dos limites de velocidade (aplicado apenas ao andar sobre uma plataforma para não restringir o movimento de lançamento pelo gancho)
    if (isGrounded_3D)
    {
    ...
    // Verificação com espinhos e porta (mais simplificada que as outras pois, até então, espinhos só ficam no chão). 
    // Não escolhemos fazer espinhos em paredes ou no teto pois já estava ficando pesada a modelagem
...
void gameDisplay_3D()
{
    ...
    for (const auto &p : platforms_3D)
    {
        // Calcula o centro da plataforma na parte de cima para escolher onde colocar o texto
    ...
    for (const auto &w : breakableWalls_3D)
    {
        ...
        // Calcula o centro da parede na parte de cima para escolher onde colocar o texto
...
/**
 * Chamada quando a janela é redimensionada. Análogo ao modo 2D, exceto pela perspectiva
 */
void gameReshape_3D(int w, int h)
{
    ...
    // Visualização de perspectiva escolhida para se adaptar ao modo 3D em 3ª pessoa
...
void gameMouseMotion_3D(int x, int y)
{
    ...
    {
        // Assim como no início da fase, fazemos o mouse ir para o centro da tela em modo FPS
    ...
    // Cálculo dinâmico para definir um limite para o usuário não olhar tanto para cima.
    // Fizemos isso pois o chão acabava entrando na frente da visualização da tela pelo personagem
    float margin = 2.0f;
    ...
    // Trava da mira em relação à tela de visualização (essa trava é diferente da trava da tela)
...
/**
 * TODO - carregar texturas e assets.
 */
